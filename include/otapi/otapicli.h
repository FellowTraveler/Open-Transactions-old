...
namespace otapicli {
	...
	public :
		...
		String ^ __clrcall NumList_Add(
			String ^% strNumList,
			String ^% strNumbers
			);

		String ^ __clrcall NumList_Remove(
			String ^% strNumList,
			String ^% strNumbers
			);

		Boolean __clrcall NumList_VerifyQuery(
			String ^% strNumList,
			String ^% strNumbers
			);

		Boolean __clrcall NumList_VerifyAll(
			String ^% strNumList,
			String ^% strNumbers
			);

		Int32 __clrcall NumList_Count(
			String ^% strNumList
			);

		String ^ __clrcall Encode(
			String ^% strPlaInt32ext,
			Boolean % bLineBreaks
			); 

		String ^ __clrcall Decode(
			String ^% strEncoded,
			Boolean % bLineBreaks
			);

		String ^ __clrcall Encrypt(
			String ^% RECIPIENT_NYM_ID,
			String ^% strPlaInt32ext
			);

		String ^ __clrcall Decrypt(
			String ^% RECIPIENT_NYM_ID,
		...
		String ^ __clrcall SymmetricEncrypt(
			String ^% SYMMETRIC_KEY,
			String ^% PLAINTEXT
			);

		String ^ __clrcall SymmetricDecrypt(
			String ^% SYMMETRIC_KEY,
			String ^% CIPHERTEXT_ENVELOPE
			);

		String ^ __clrcall SignContract(
			String ^% SIGNER_NYM_ID,
			String ^% THE_CONTRACT
			);

		String ^ __clrcall AddSignature(
			String ^% SIGNER_NYM_ID,
			String ^% THE_CONTRACT
			);

		Boolean __clrcall VerifySignature(
			String ^% SIGNER_NYM_ID,
			String ^% THE_CONTRACT
			);

		String ^ __clrcall VerifyAndRetrieveXMLContents(
			String ^% THE_CONTRACT,
			String ^% SIGNER_ID
			);
		...
		String ^ __clrcall CreateServerContract(
			String ^% NYM_ID,
			String ^% strXMLcontents
			);

		String ^ __clrcall CreateAssetContract(
			String ^% NYM_ID,
			String ^% strXMLcontents
			);

		Boolean __clrcall AddServerContract(String ^% strContract);

		Boolean __clrcall AddAssetContract(String ^% strContract);

		Int32 __clrcall GetServerCount(void);
		Int32 __clrcall GetAssetTypeCount(void);
		Int32 __clrcall GetAccountCount(void);
		Int32 __clrcall GetNymCount(void);
		...
		Boolean __clrcall VerifyAccountReceipt(
			String ^% SERVER_ID,
			String ^% NYM_ID,
			String ^% ACCT_ID
			);

		Int32 __clrcall GetNym_TransactionNumCount(
			String ^% SERVER_ID,
			String ^% NYM_ID
			);
		...
		String ^ __clrcall GetNym_NymboxHash(
			String ^% SERVER_ID,
			String ^% NYM_ID 
			);

		String ^ __clrcall GetNym_RecentHash(
			String ^% SERVER_ID,
			String ^% NYM_ID
			);

		String ^ __clrcall GetNym_InboxHash(
			String ^% ACCOUNT_ID,
			String ^% NYM_ID
			);

		String ^ __clrcall GetNym_OutboxHash(
			String ^% ACCOUNT_ID,
			String ^% NYM_ID
			);
		...
		String ^ __clrcall GetNym_MailContentsByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		String ^ __clrcall GetNym_MailSenderIDByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		String ^ __clrcall GetNym_MailServerIDByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		Boolean __clrcall Nym_RemoveMailByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		Boolean __clrcall Nym_VerifyMailByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);
		...
		String ^ __clrcall GetNym_OutmailContentsByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		String ^ __clrcall GetNym_OutmailRecipientIDByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		String ^ __clrcall GetNym_OutmailServerIDByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		Boolean __clrcall Nym_RemoveOutmailByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		Boolean __clrcall Nym_VerifyOutmailByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);
		...
		String ^ __clrcall GetNym_OutpaymentsContentsByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		String ^ __clrcall GetNym_OutpaymentsRecipientIDByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		String ^ __clrcall GetNym_OutpaymentsServerIDByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		Boolean __clrcall Nym_RemoveOutpaymentsByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);

		Boolean __clrcall Nym_VerifyOutpaymentsByIndex(
			String ^% NYM_ID,
			Int32 % nIndex
			);
		...
		Int64 __clrcall Ledger_GetTransactionNumberByIndex(
			String ^% SERVER_ID,
			String ^% USER_ID,
			String ^% ACCOUNT_ID,
			String ^% THE_LEDGER,
		...
		Int32 __clrcall Transaction_GetSuccess(
			String ^% SERVER_ID,
			String ^% USER_ID,
			String ^% ACCOUNT_ID,
			String ^% THE_TRANSACTION
		...
		Int32 __clrcall Transaction_GetBalanceAgreementSuccess(
			String ^% SERVER_ID,
			String ^% USER_ID,
			String ^% ACCOUNT_ID,
			String ^% THE_TRANSACTION
		...
		DateTime Transaction_GetDateSigned(
			String ^% SERVER_ID,
			String ^% USER_ID,
			String ^% ACCOUNT_ID,
			String ^% THE_TRANSACTION
		...
		DateTime Token_GetValidFrom(
			String ^% SERVER_ID,
			String ^% ASSET_TYPE_ID,
			String ^% THE_TOKEN
			);

		DateTime Token_GetValidTo(
			String ^% SERVER_ID,
			String ^% ASSET_TYPE_ID,
			String ^% THE_TOKEN
			);
		...
		Int64 __clrcall Instrument_GetAmount(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		Int64 __clrcall Instrument_GetTransNum(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		DateTime Instrument_GetValidFrom(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		DateTime Instrument_GetValidTo(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		String ^ __clrcall Instrument_GetMemo(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		String ^ __clrcall Instrument_GetType(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		String ^ __clrcall Instrument_GetAssetID(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		String ^ __clrcall Instrmnt_GetSenderUserID(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		String ^ __clrcall Instrmnt_GetSenderAcctID(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		String ^ __clrcall Instrmnt_GetRecipientUserID(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		String ^ __clrcall Instrmnt_GetRecipientAcctID(
			String ^% SERVER_ID,
			String ^% THE_INSTRUMENT
			);

		Int32 __clrcall checkServerID(
			String ^% SERVER_ID,
			String ^% USER_ID
			);
		...
		Int32 __clrcall getNym_MarketOffers(
			String ^% SERVER_ID,
			String ^% USER_ID
			); 
		...
		void Sleep(String ^% MILLISECONDS);

		Boolean __clrcall ResyncNymWithServer(
			String ^% SERVER_ID,
			String ^% USER_ID,
			String ^% THE_MESSAGE
			);
		...
		Int32 __clrcall queryAssetTypes(
			String ^% SERVER_ID,
			String ^% USER_ID,
			String ^% ENCODED_MAP
			);
		...
		Int64 __clrcall Message_GetDepth(String ^% THE_MESSAGE);
