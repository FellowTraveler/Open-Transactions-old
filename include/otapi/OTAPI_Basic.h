...
Every-other type will be wraped in a std::string.
...
#ifndef __OT_API_BASIC_H__
#define __OT_API_BASIC_H__
...
public :
	...
	*/
	...
	longerface. You would use Java functions at that polong.)
	...
	*/
	...
	// of long longegers, stored in a std::set and easily serializable in/out of a string.
	//(It's useful.)
	//


	EXPORT	static	string NumList_Add(
		const string & strNumList,
		const string & strNumbers
		);

	EXPORT	static	string NumList_Remove(
		const string & strNumList,
		const string & strNumbers
		);
	...
	EXPORT	static	long NumList_Count(
		const string & strNumList
		);
	...
	string Encode(const string & strPlalongext);

	This will pack, compress, and base64-encode a plain string.
	Returns the base64-encoded string, or NULL.

	longernally: 
	OTString strPlain(strPlalongext);
	OTASCIIArmor ascEncoded(thePlalongext); // ascEncoded now contains the OT-encoded string.
	return ascEncoded.Get(); // We return it.
	*/
	EXPORT	static	string Encode(
		const string & strPlalongext,
		bool bLineBreaks
		); 
	...
	*/
	...
	Decrypts the base64-encoded ciphertext back longo a normal string PlainText.
	Returns the PlainText string, or NULL.
	...
	if(theEnvelope.SetAsciiArmoredData(ascCiphertext)) { // ...so that we can open it using the appropriate Nym, longo a plain string object:
	...
	*/
	...
	signs longernally wherever it deems appropriate. Thus, this function is only for
	...
	*/
	...
	EXPORT	static	string GetMemlogAtIndex(const long nIndex);
	...
	//
	EXPORT	static	string CreateNym(const long nKeySize); // must be 1024, 2048, 4096, or 8192 
	...
	*/
	...
	//
	EXPORT	static	long GetServerCount(void);
	EXPORT	static	long GetAssetTypeCount(void);
	EXPORT	static	long GetAccountCount(void);
	EXPORT	static	long GetNymCount(void);



	EXPORT	static	string GetServer_ID(const long nIndex); // based on Index(above 4 functions) this returns the Server's ID
	...
	EXPORT	static	string GetAssetType_ID(const long nIndex); // returns Asset Type ID(based on index from GetAssetTypeCount)
	...
	EXPORT	static	string GetAccountWallet_ID(const long nIndex);  // returns a string containing the account ID, based on index.
	EXPORT	static	string GetAccountWallet_Name(const string & ACCOUNT_ID);  // returns the account name, based on account ID.
	EXPORT	static	string GetAccountWallet_Balance(const string & ACCOUNT_ID);  // returns the account balance, based on account ID.
	...
	// Verifies any asset account(const longermediary files) against its own last signed receipt.
	...
	//
	EXPORT	static	long GetNym_TransactionNumCount(
		const string & SERVER_ID,
		const string & NYM_ID
		);

	EXPORT	static	string GetNym_ID(const long nIndex); // based on Index(above 4 functions) this returns the Nym's ID
	...
	*/
	...
	EXPORT	static	long GetNym_MailCount(const string & NYM_ID);
	...
	EXPORT	static	long GetNym_OutmailCount(const string & NYM_ID);
	...
	EXPORT	static	long GetNym_OutpaymentsCount(const string & NYM_ID);
	...
	==> WriteCheque() longernally constructs an OTCheque 
	...
	// will convert to a long longeger. Negative amounts 
	...
	*/
	...
	const string & VALID_FROM=0, const string & VALID_TO=0);

	----------------------------------------------------------------------------------------
	(Optional initial payment):
	bool OTPaymentPlan::SetInitialPayment(const long & lAmount, string tTimeUntilInitialPayment=0); // default: now.
	----------------------------------------------------------------------------------------
	...
	string tTimeUntilPlanStart=LENGTH_OF_MONTH_IN_SECONDS, // Default: 1st payment in 30 days
	string tBetweenPayments=LENGTH_OF_MONTH_IN_SECONDS, // Default: 30 days.
	string tPlanLength=0, long nMaxPayments=0);
	----------------------------------------------------------------------------------------
	*/
	EXPORT	static	string ProposePaymentPlan(
		const string & SERVER_ID,
		// ----------------------------------------
		const string & VALID_FROM, // Default(0 or NULL) == NOW
		const string & VALID_TO, // Default(0 or NULL) == no expiry / cancel anytime
		// ----------------------------------------
		...
		const long & PAYMENT_PLAN_MAX_PAYMENTS // longeger. Defaults to 0 or NULL(no maximum payments.)
		);
	...
	EXPORT	static	string Create_SmartContract(
		const string & SERVER_ID,
		const string & SIGNER_NYM_ID,// Use any Nym you wish here.(The signing at this polong is only to cause a save.)
		// ----------------------------------------
		const string & VALID_FROM, // Default(0 or NULL) == NOW
		const string & VALID_TO // Default(0 or NULL) == no expiry / cancel anytime
		);
	...
	// >0 means NO error, and the message was sent, and the request number fits longo an longeger...
	...
	*/
	...
	///Returns OT_BOOL
	//long HarvestClosingNumbers(const string & SERVER_ID,
	//   string NYM_ID,
	...
	//
	///Returns OT_BOOL
	//long HarvestAllNumbers(const string & SERVER_ID,
	//   string NYM_ID,
	//   string THE_CRON_ITEM);
	...
	// --------------------------------------------------------------
	// LOAD PURSE or Mlong or ASSET CONTRACT or SERVER CONTRACT --(from local storage)
	//
	// Based on Asset Type ID: load a purse, a public mlong, or an asset/server contract
	...
	// TRUE if the mlong is still usable.
	...
	//(Or zero.)
	//
	EXPORT	static	long Basket_GetMemberCount(const string & BASKET_ASSET_TYPE_ID);
	...
	//
	EXPORT	static	string Basket_GetMinimumTransferAmount(const string & BASKET_ASSET_TYPE_ID);
	...
	//
	EXPORT	static	string Basket_GetMemberMinimumTransferAmount(
		const string & BASKET_ASSET_TYPE_ID,
		const long & nIndex
		);
	...
	EXPORT	static	string Ledger_GetTransactionNumberByIndex(
		const string & SERVER_ID,
		const string & USER_ID,
		const string & ACCOUNT_ID,
		const string & THE_LEDGER,
	...
	*/
	...
	// --------------------------------------------------------------------
	// Get Transaction Type(const longernally uses GetTransactionTypestring().)
	//
	...
	//
	EXPORT	static	string ReplyNotice_GetRequestNum(
		const string & SERVER_ID,
		const string & USER_ID,
		const string & THE_TRANSACTION
		);
	...
	//
	// If you withdrew longo a voucher instead of cash, this function allows
	// you to retrieve the actual voucher cheque from the reply transaction.
	//(A voucher is a cheque drawn on an longernal server account instead
	...
	// --------------------------------------------------
	//
	// Get Transaction Date Signed(const longernally uses OTTransaction::GetDateSigned().)
	//
	EXPORT	static	string Transaction_GetDateSigned(
		const string & SERVER_ID,
		const string & USER_ID,
		const string & ACCOUNT_ID,
		const string & THE_TRANSACTION
	...
	EXPORT	static	string Transaction_GetAmount(
		const string & SERVER_ID,
		const string & USER_ID,
		const string & ACCOUNT_ID,
		const string & THE_TRANSACTION
	...
	//
	EXPORT	static	string Transaction_GetDisplayReferenceToNum(
		const string & SERVER_ID,
		const string & USER_ID,
		const string & ACCOUNT_ID,
		const string & THE_TRANSACTION
	...
	*/
	...
	// then IMPORT whatever other purse you want longo it, then
	// SAVE it again.
	// returns OT_BOOL
	...
	// --------------------------------------------------------------------
	// Get Purse Total Value(const longernally uses GetTotalValue().)
	//
	// Returns the purported sum of all the tokens within.
	//
	EXPORT	static	string Purse_GetTotalValue(
		const string & SERVER_ID,
		const string & ASSET_TYPE_ID,
		const string & THE_PURSE
		);
	...
	// If success, the new tokens will be returned shortly and saved longo the appropriate purse.
	...
	// The actual cash value of the token. Returns a long long as a string.
	//
	EXPORT	static	string Token_GetDenomination(
		const string & SERVER_ID,
		const string & ASSET_TYPE_ID,
		const string & THE_TOKEN
		);
	...
	//
	EXPORT	static	string Token_GetValidFrom(
		const string & SERVER_ID,
		const string & ASSET_TYPE_ID,
		const string & THE_TOKEN
		);
	...
	// the date is seconds since Jan 1970, but returned as a string.
	//
	EXPORT	static	string Token_GetValidTo(
		const string & SERVER_ID,
		const string & ASSET_TYPE_ID,
		const string & THE_TOKEN
		);
	...
	// SMART CONTRACTS, and PURSEs longo these functions, and they should be able
	// to handle any of those types.
	//
	//

	EXPORT	static	string Instrument_GetAmount(
		const string & SERVER_ID,
		const string & THE_INSTRUMENT
		);

	EXPORT	static	string Instrument_GetTransNum(
		const string & SERVER_ID,
		const string & THE_INSTRUMENT
		);

	EXPORT	static	string Instrument_GetValidFrom(
		const string & SERVER_ID,
		const string & THE_INSTRUMENT
		);

	EXPORT	static	string Instrument_GetValidTo(
		const string & SERVER_ID,
		const string & THE_INSTRUMENT
		);
	...
	(That's in HTTP mode -- in TCP mode it malongains the connection.)
	...
	*/
	...
	// ===> In 99% of cases, this LAST option is what actually happens!!
	//
	EXPORT	static	long checkServerID(
		const string & SERVER_ID,
		const string & USER_ID
		);
	...
	been created by this polong, you simply pass in their IDs and the
	...
	*/
	...
	down(+ or - any long long value in string format). If you do, the server reply will
	...
	// ---------------------------
	After you call usageCredits(), you will receive a server reply. Pass that longo
	...
	*/
	...
	// then this function returns the usage credits BALANCE(it's a long long, but
	...
	// 
	EXPORT	static	string Message_GetUsageCredits(const string & THE_MESSAGE);
	...
	*/
	...
	go through! This mechanism prevents an attack from longercepting a message
	...
	*/
	...
	// --------------------------------------------------------------------------
	// Get server's copy of public Mlong file(which contains the public mlonging
	// keys for each asset type. Withdrawal requests will not work for any given
	// asset type until you have downloaded the mlong for that asset type.)
	//
	// Returns long:
	// -1 means error; no message was sent.
	// -2 means the message was sent, but the request number must be passed as a string, so call GetLargeRequestNum.
	...
	// issuer account. But with baskets, that is managed longernally by the server.
	...
	// ask the server to convert from your gold, silver, and dollar accounts longo
	// your Rand account.(Or convert from your Rand account back longo your gold,
	// silver, and dollar accounts.)
	//
	// Other than this conversion process,(with the reserves stored longernally by
	...
	// will automatically save the tokens in that purse longo your purse
	...
	// function to actually deposit that cash longo a server account.(The cash
	...
	// dropped longo their respective inboxes.
	//
	// Returns long:
	// -1 means error; no message was sent.
	// -2 means the message was sent, but the request number must be passed as a string, so call GetLargeRequestNum.
	...
	will automatically process that longo a processInbox command back to the server, 
	...
	*/
	...
	You will probably ask me for more longrospection on the transactions themselves. 
	...
	*/
	...
	// longo your Nymbox with a copy of the server's reply. It's called a replyNotice.
	...
	// how it was longended.) But after that, it will no longer know if you got the reply since
	...
	// Once you download your nym/in/out/*box and load it longo memory from storage, you iterate through
	// it and download all the box receipts(preferably only once.) Once you load those up, it's as if
	// the various pieces were never sundered longo multiple files to begin with. Then you can verify
	...
	*/
	...
	//
	// Basically the funds are moved longo an longernal server account, and then
	...
	// --------------------------------------------------------------------------
	// DEPOSIT A CHEQUE(or VOUCHER) longO AN ASSET ACCOUNT.
	//
	// If someone gives you a cheque, you can use this function to message the
	// server asking it to deposit the cheque longo one of your asset accounts.
	//(Of course the account and the cheque must be the same asset type.)
	//
	// Since a voucher is simply a cheque drawn on an longernal server account,
	...
	polongers to the elements as needed. THERE IS ONLY ONE OF THESE.
	...
	*/
	...
	// ===> In 99% of cases, this LAST option is what actually happens!!
	//
	EXPORT	static	long getMarketList(
		const string & SERVER_ID,
		const string & USER_ID
		);
	...
	// ===> In 99% of cases, this LAST option is what actually happens!!
	//
	EXPORT	static	long getNym_MarketOffers(
		const string & SERVER_ID,
	...
	*/
	...
	// and if you send "getMlong" the reply is "@getMlong", and so on.
	//
	...
	//
	EXPORT	static	long Message_GetSuccess(const string & THE_MESSAGE);
	...
	// ===> In 99% of cases, this LAST option is what actually happens!!
	//
	EXPORT	static	long queryAssetTypes(
		const string & SERVER_ID,
		const string & USER_ID,
	...
	// reply.)
	//
	EXPORT	static	string Message_GetDepth(const string & THE_MESSAGE);
