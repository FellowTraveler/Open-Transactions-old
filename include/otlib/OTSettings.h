...
public:
	...
	EXPORT	const bool		IsEmpty();

	// Check Only	(get value of key from configuration, if the key exists, then out_bKeyExist will be true.)
	EXPORT	const bool		Check_str (const OTString & strSection, const OTString & strKey, OTString & out_strResult,	bool & out_bKeyExist);
	EXPORT	const bool		Check_long(const OTString & strSection, const OTString & strKey, long	  & out_lResult,	bool & out_bKeyExist);
	EXPORT	const bool		Check_bool(const OTString & strSection, const OTString & strKey, bool     &	out_bResult,	bool & out_bKeyExist);
