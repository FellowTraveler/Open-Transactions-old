...
private:
	...
	const bool HandleSendingError();
	const bool HandleReceivingError();
...
public:
	...
	const bool Init(const std::unique_ptr<OTSettings> & pSettings);

	const bool NewContext();

	//const bool Listen(); // assumes m_strBindPath is already set.
	...
	const bool &		IsIsListening()		const { return m_bIsListening;	  }
	const OTString & CurrentBindingPath()	const { return m_strBindingPath;  }
};
...
const bool OTSocket::NewContext()
{
	...
	OT_ASSERT_MSG(nullptr != m_pContext, "OTSocket::NewContext():  Failed creating network context: zmq::context_t * pContext = new zmq::context_t(1);");
...
//const bool OTSocket::Listen() // assumes m_strBindPath is already set.
//{
...
const bool OTSocket::Listen(const OTString &strBind)  // strBind is optional
{
	if (!m_bInitialized) return false;
	if (!m_HasContext) return false;
	m_bIsListening = false;
	...
	m_pSocket = std::unique_ptr<zmq::socket_t>(new zmq::socket_t(*m_pContext, ZMQ_REP));  // RESPONSE socket (Request / Response.)
...
const bool OTSocket::HandlePollingError()
{
	...
	switch (errno) {
		...
		case ETERM:
			...
			this->NewContext();
			this->Listen();  // will fail if m_strBindingPath isn't already set.
			break;		
...
{
	OT_ASSERT_MSG(nullptr != m_pSocket, "m_pSocket == NULL in OTSocket::Send()");
...
//	SSL_load_error_strings();
	...
	{
	const std::unique_ptr<OTSettings> pSettings(new OTSettings(OTDataFolder::GetConfigFilePath()));
