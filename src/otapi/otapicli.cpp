...
{
	set<long> setNumList;
	if (!strNumList -> Empty)  setNumList = OTAPI_Wrap::NumList(Native(strNumList));
	else setNumList.clear();

	set<long> setNumbers = OTAPI_Wrap::NumList(Native(strNumbers));

	set<long> setResult = OTAPI_Wrap::NumList_Add(setNumList,setNumbers);

	return Managed(OTAPI_Wrap::NumList_str(setResult));
}
...
{
	set<long> setNumList = OTAPI_Wrap::NumList(Native(strNumList));
	set<long> setNumbers = OTAPI_Wrap::NumList(Native(strNumbers));

	set<long> setResult = OTAPI_Wrap::NumList_Remove(setNumList,setNumbers);
...
Boolean otapi_wrap::AddServerContract(String ^% strContract)
{
	return OTAPI_Wrap::AddServerContract(Native(strContract));
}

Boolean otapi_wrap::AddAssetContract(String ^% strContract)
{
...
{
	return OTAPI_Wrap::GetAccountWallet_Balance(Native(ACCOUNT_ID));
}
...
String ^ otapi_wrap::ProposePaymentPlan(
	...
	String ^% INITIAL_PAYMENT_DELAY,
	String ^% PAYMENT_PLAN_AMOUNT,
	String ^% PAYMENT_PLAN_DELAY,
	String ^% PAYMENT_PLAN_PERIOD,
	String ^% PAYMENT_PLAN_LENGTH,
	Int32 % PAYMENT_PLAN_MAX_PAYMENTS
	)
{
	//String n_INITIAL_PAYMENT_AMOUNT = INITIAL_PAYMENT_AMOUNT;
	//String n_INITIAL_PAYMENT_DELAY = INITIAL_PAYMENT_DELAY;
	//String n_PAYMENT_PLAN_AMOUNT = PAYMENT_PLAN_AMOUNT;
	//String n_PAYMENT_PLAN_DELAY = PAYMENT_PLAN_DELAY;
	//String n_PAYMENT_PLAN_PERIOD = PAYMENT_PLAN_PERIOD;
	//String n_PAYMENT_PLAN_LENGTH = PAYMENT_PLAN_LENGTH;
	Int32 n_PAYMENT_PLAN_MAX_PAYMENTS = PAYMENT_PLAN_MAX_PAYMENTS;
...
{
	Int32 n_nIndex = nIndex;
	return OTAPI_Wrap::Basket_GetMemberMinimumTransferAmount(Native(BASKET_ASSET_TYPE_ID),n_nIndex);
}
...
Int64 otapi_wrap::Ledger_GetTransactionNumberByIndex(
	String ^% SERVER_ID,
	String ^% USER_ID,
	String ^% ACCOUNT_ID,
	String ^% THE_LEDGER,
...
{
	Int32 n_nIndex = nIndex;
	return OTAPI_Wrap::Ledger_GetTransactionNumberByIndex(Native(SERVER_ID), Native(USER_ID), Native(ACCOUNT_ID), Native(THE_LEDGER),n_nIndex);
}
...
Int32 otapi_wrap::Transaction_GetSuccess(
	String ^% SERVER_ID,
	String ^% USER_ID,
	String ^% ACCOUNT_ID,
	String ^% THE_TRANSACTION
...
Int32 otapi_wrap::Transaction_GetBalanceAgreementSuccess(
	String ^% SERVER_ID,
	String ^% USER_ID,
	String ^% ACCOUNT_ID,
	String ^% THE_TRANSACTION
...
{
	return OTAPI_Wrap::Transaction_GetAmount(Native(SERVER_ID), Native(USER_ID), Native(ACCOUNT_ID), Native(THE_TRANSACTION));
}
...
String ^ otapi_wrap::CreatePurse(
	String ^% SERVER_ID,
	String ^% ASSET_TYPE_ID,
	String ^% USER_ID // UserID optional.
	)
{
	return Managed(OTAPI_Wrap::CreatePurse(Native(SERVER_ID), Native(ASSET_TYPE_ID), Native(USER_ID)));
}
...
{
	return Managed(OTAPI_Wrap::Purse_Push(Native(SERVER_ID), Native(ASSET_TYPE_ID), Native(USER_ID), Native(THE_PURSE), Native(THE_TOKEN)));
}
...
String ^ otapi_wrap::Token_ChangeOwner(
	...
	String ^% ASSET_TYPE_ID,
	String ^% THE_TOKEN,
	String ^% OLD_OWNER_NYM_ID,
	String ^% NEW_OWNER_NYM_ID
	)
{
	return Managed(OTAPI_Wrap::Token_ChangeOwner(Native(SERVER_ID), Native(ASSET_TYPE_ID), Native(THE_TOKEN), Native(OLD_OWNER_NYM_ID), Native(NEW_OWNER_NYM_ID)));
}
...
Int64 otapi_wrap::Instrument_GetAmount(
	String ^% SERVER_ID,
	String ^% THE_INSTRUMENT
	)
{
	return OTAPI_Wrap::Instrument_GetAmount(Native(SERVER_ID), Native(THE_INSTRUMENT));
}

Int64 otapi_wrap::Instrument_GetTransNum(
	String ^% SERVER_ID,
	String ^% THE_INSTRUMENT
	)
{
	return OTAPI_Wrap::Instrument_GetTransNum(Native(SERVER_ID), Native(THE_INSTRUMENT));
}

DateTime otapi_wrap::Instrument_GetValidFrom(
	String ^% SERVER_ID,
	String ^% THE_INSTRUMENT
	)
{
	return Managed(OTAPI_Wrap::Instrument_GetValidFrom(Native(SERVER_ID), Native(THE_INSTRUMENT)));
}

DateTime otapi_wrap::Instrument_GetValidTo(
	String ^% SERVER_ID,
	String ^% THE_INSTRUMENT
	)
{
	return Managed(OTAPI_Wrap::Instrument_GetValidTo(Native(SERVER_ID), Native(THE_INSTRUMENT)));
}
...
{
	return Managed(OTAPI_Wrap::Instrument_GetMemo(Native(SERVER_ID), Native(THE_INSTRUMENT)));
}
...
{
	return Managed(OTAPI_Wrap::Instrument_GetType(Native(SERVER_ID), Native(THE_INSTRUMENT)));
}
...
{
	return Managed(OTAPI_Wrap::Instrument_GetAssetID(Native(SERVER_ID), Native(THE_INSTRUMENT)));
}
...
{
	return Managed(OTAPI_Wrap::Instrmnt_GetSenderUserID(Native(SERVER_ID), Native(THE_INSTRUMENT)));
}
...
{
	return Managed(OTAPI_Wrap::Instrmnt_GetSenderAcctID(Native(SERVER_ID), Native(THE_INSTRUMENT)));
}
...
{
	return Managed(OTAPI_Wrap::Instrmnt_GetRecipientUserID(Native(SERVER_ID), Native(THE_INSTRUMENT)));
}
...
{
	return Managed(OTAPI_Wrap::Instrmnt_GetRecipientAcctID(Native(SERVER_ID), Native(THE_INSTRUMENT)));
}
...
Int32 otapi_wrap::issueMarketOffer(
	...
	String ^% MARKET_SCALE,
	String ^% MINIMUM_INCREMENT,
	String ^% TOTAL_ASSETS_ON_OFFER,
	String ^% PRICE_LIMIT,
...
void otapi_wrap::Sleep(String ^% MILLISECONDS)
{
...
Int64 otapi_wrap::Message_GetDepth(String ^% THE_MESSAGE) { return OTAPI_Wrap::Message_GetDepth(Native(THE_MESSAGE)); }
