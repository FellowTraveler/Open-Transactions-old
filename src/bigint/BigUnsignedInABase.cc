...
BigUnsignedInABase::BigUnsignedInABase(const std::string &s, Base base) {
	// Check the base.
	if (base > 62)
		throw "BigUnsignedInABase(std::string, Base): The default string conversion routines use the symbol set 0-9, A-Z, a-z, and therefore support only up to base 62.  You tried a conversion with a base over 62; write your own string conversion routine.";
	// Save the base.
...
BigUnsignedInABase::operator std::string() const {
	if (base > 62)
		throw "BigUnsignedInABase ==> std::string: The default string conversion routines use the symbol set 0-9, A-Z, a-z, and therefore support only up to base 62.  You tried a conversion with a base over 62; write your own string conversion routine.";
	if (len == 0)
		return std::string("0");
	...
	for (symbolNumInString = 0; symbolNumInString < len; symbolNumInString++) {
		...
		if (theDigit < 10)
			...
			s[symbolNumInString] = char('a' + theDigit - 36);
	}
	std::string s2(s);
	delete [] s;
	return s2;
